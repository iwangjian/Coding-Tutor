...
class GPTChat(IntelligenceBackend):
    """
...
class O1Chat(IntelligenceBackend):
    """
    Interface to the o1 style model with user, assistant roles separation
    """
    stateful = False
    type_name = "o1-chat"
    ...
    def __init__(self, 
        ...
        super().__init__(
            model=model,max_latest_messages=max_latest_messages, **kwargs)
    ...
    def query(self, agent_name: str, role_desc: str, history_messages: List[Message], global_prompt: str = None,
        ...
        """
        format the input and call the o1-like API
        args:
            agent_name: the name of the agent
            role_desc: the description of the role of the agent
            history_messages: the history of the conversation, or the observation for the agent
        ...
        for i, msg in enumerate(all_messages):
            if i == 0:
                assert msg[0] == SYSTEM_NAME  # The first message should be from the assistant
                messages.append({"role": "assistant", "content": msg[1]})
            elif i == len(all_messages) - 1:
                assert msg[0] == SYSTEM_NAME  # The last message should be from the assistant
                messages[-1]["content"] = f"{messages[-1]['content']}\n\n[{msg[0]}]: {msg[1]}"
            else:
                if msg[0] == agent_name:
                    messages.append({"role": "assistant", "content": msg[1]})
