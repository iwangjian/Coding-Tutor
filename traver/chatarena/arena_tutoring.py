...
from .environments import TutoringConversation, TimeStep, load_environment
...
class TutoringArena:
    ...
    def __init__(self, players: List[Player], environment: TutoringConversation, global_prompt: str = None):
    ...
    def step(self) -> TimeStep:
        ...
        for i in range(self.invalid_actions_retry):  # try to take an action for a few times
            if player_name == "Tutor":
                (action, student_knowledge, response_list, verifier_scores) = player(observation)  # take an action
            else:
